...
RUN /usr/bin/wget --continue https://github.com/splitbrain/dokuwiki/archive/${DOKUWIKI_INSTALL} && \
  /bin/tar xvfz /usr/share/dokuwiki/${DOKUWIKI_INSTALL} && \
  DOKUWIKI_INSTALLED=`/usr/bin/find . -type d -maxdepth 1 | /bin/grep doku` && \
  /bin/rmdir /var/www/localhost/htdocs && \
  /bin/ln -s /usr/share/dokuwiki/${DOKUWIKI_INSTALLED} /var/www/localhost/htdocs && \
  /bin/chown -R lighttpd:lighttpd /usr/share/dokuwiki/${DOKUWIKI_INSTALLED}
